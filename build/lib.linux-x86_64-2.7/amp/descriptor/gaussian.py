    ...
    def calculate_fingerprints(self, images, parallel=None, log=None,
        ...
        for element, fingerprints in p.Gs.items():
            ...
            for index, fp in enumerate(fingerprints):
                if fp['type'] == 'G2':
                    log(' {}: {}, {}, eta = {}, Rs = {}'
                        .format(index, fp['type'], fp['element'], fp['eta'], fp['Rs']))
                elif fp['type'] == 'G4':
                    log(' {}: {}, ({}, {}), eta={}, gamma={}, zeta={}, theta_s={}'
...
class FingerprintCalculator:
    ...
    Gs : dict
        ...
        example:
               ...
               >>> Gs = {"O": [{"type":"G2", "element":"O", "eta":10., "Rs": 0.0},
               ...             {"type":"G4", "elements":["O", "Au"],
               ...              "eta":5., "gamma":1., "zeta":1.0, "theta_s": 1}],
               ...       "Au": [{"type":"G2", "element":"O", "eta":2.},
               ...              {"type":"G4", "elements":["O", "Au"],
               ...               "eta":2., "gamma":1., "zeta":5.0}]}
    ...
    def calculate(self, image, key):
        ...
        for atom in image:
            ...
            print "neighborsymbols:", neighborsymbols
            print "neighborpositions", neighborpositions
...
#        print len(self.globals.Gs[symbol])
        num_symmetries = len(self.globals.Gs[symbol])
        fingerprint = [None] * num_symmetries
        neighbornumbers = [atomic_numbers[nsymbol] for nsymbol in neighborsymbols]
...
#            print "symbol:",count
            ...
            if G['type'] == 'G2':
                ...
                ridge = calculate_G4(neighbornumbers, neighborsymbols, neighborpositions,
                                     G['elements'], G['gamma'],
                                     G['zeta'], G['eta'], G['theta_s'], self.globals.cutoff,
                                     Ri, self.fortran)
            elif G['type'] == 'G5':
...
def calculate_G2(neighbornumbers, neighborsymbols, neighborpositions, G_element, eta, Rs, cutoff, Ri, fortran):
...
def calculate_G4(neighbornumbers, neighborsymbols, neighborpositions,
                 G_elements, gamma, zeta, eta, theta_s, cutoff,
                 Ri, fortran):
    ...
    eta : float
        Parameter of Gaussian symmetry functions.
    theta_s : list (Lei added)
        shift the center of the G4 function
    cutoff : dict
        Cutoff function, typically from amp.descriptor.cutoffs. Should be also
        formatted as a dictionary by todict method, e.g.
        cutoff=Cosine(6.5).todict()
