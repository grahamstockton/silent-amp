...
if (comp_forces.eq.1) then
  ...
  ! Lei modified for different number of species
  allocate(fp_range(buf%num_species))
  do p=1,buf%num_species
    allocate(fp_range(p)%min_fingerprints(size(buf%symmetry_functions(p)%gs)))
    allocate(fp_range(p)%max_fingerprints(size(buf%symmetry_functions(p)%gs)))
    ! write(*,*)shape(fp_range(p)%min_fingerprints)
  end do
  ...
  do p = 1, buf%num_species
    do q = 1, size(buf%symmetry_functions(p)%gs)
      ...
      fp_range(p)%min_fingerprints(q) = buf%symmetry_functions(p)%gs(q)%min_fingerprint
      fp_range(p)%max_fingerprints(q) = buf%symmetry_functions(p)%gs(q)%max_fingerprint
    end do
  end do
...
if (comp_forces.eq.1) then
  ...
  do selfindex = 1, num_atoms
   ...
   ! if (particle_contributing(selfindex) == 1) then
      ...
      !Lei added
      !write(*,*) "forces:",forces(1,selfindex),forces(2,selfindex),&
      !forces(3,selfindex)
   ! end if
  end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do p=1,buf%num_species
    deallocate(fp_range(p)%min_fingerprints)
    deallocate(fp_range(p)%max_fingerprints)
  end do
  deallocate(fp_range)
...
do q = 1, num_gs
  ...
  if (g%symmetry_type == 'g2') then
    ...
    g%eta, rc, g%rs, cutofffn_code, i, ri, m, l, g%rmin, number_of_neighbors, ridge)
    ...
    g_numbers, g%gamma, g%zeta, g%eta, rc, g%theta_s, cutofffn_code, i, ri, m, l, number_of_neighbors, ridge)
...
do q = 1, num_gs
  ...
  if (g%symmetry_type == 'g2') then
    ...
    g%species1_code, g%eta, g%gamma, rc, g%rs, cutofffn_code, ri, g%rmin, number_of_neighbors, ridge)
  else if (g%symmetry_type == 'g4') then
    g_numbers(1) = g%species1_code
    g_numbers(2) = g%species2_code
    call calculate_g4(neighbor_numbers, neighbor_positions, &
    g_numbers, g%gamma, g%zeta, g%eta, rc, g%theta_s, cutofffn_code, ri, number_of_neighbors, ridge)
  else if (g%symmetry_type == 'g5') then
    g_numbers(1) = g%species1_code
    g_numbers(2) = g%species2_code
...
do p = 1, buf%num_species
  ...
  do q = 1, buf%num_symmetry_functions_of_species(p)
    ...
    if (buf%symmetry_functions(p)%gs(q)%symmetry_type == 'g2') then
      ...
      buf%symmetry_functions(p)%gs(q)%rs,  &
      buf%symmetry_functions(p)%gs(q)%rmin
      ...
      buf%symmetry_functions(p)%gs(q)%theta_s
      do k = 1, buf%num_species
        if (buf%species_name_strings(k) == buf%symmetry_functions(p)%gs(q)%species1_name_string) then
          exit
        end if
