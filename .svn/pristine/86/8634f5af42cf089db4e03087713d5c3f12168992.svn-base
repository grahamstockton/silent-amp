...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       g_number, g_eta, p_gamma, rc, rs, cutofffn_code, ri, rmin, num_neighbors, ridge)
              ...
              double precision::  g_eta, rc, rs, rmin
...
!f2py         intent(in):: g_eta, rc, rs, ri, p_gamma
...
!f2py         intent(out):: ridge
              ...
              do j = 1, num_neighbors
                  ...
                  if (match == 1) then
                    ...
                    !Lei added to avoid two atoms come too close
                    if (Rij < rmin) then
                       Rij = rmin
                    endif

                    term = exp(-g_eta*( (Rij - rs)**2.0d0) / (rc ** 2.0d0))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      g_numbers, g_gamma, g_zeta, g_eta, rc, theta_s, cutofffn_code, ri, &
              ...
              do j = 1, num_neighbors
                do k = (j + 1), num_neighbors
                  ...
                  if (match == 1) then
                    ...
                    term = (1.0d0 + g_gamma * cos(acos(costheta)-theta_s))**g_zeta
                    term = term*&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       neighborpositions, g_number, g_eta, rc, rs, cutofffn_code, i, ri, m, l, &
       rmin, num_neighbors, ridge, p_gamma)
...
!f2py         intent(in):: g_eta, rc, rs, i, ri, m, l, p_gamma
!f2py         intent(hide):: num_neighbors
!f2py         intent(out):: ridge
              integer:: j, match, xyz
              double precision, dimension(3):: Rij_vector
              ...
              do j = 1, num_neighbors
                  ...
                  if (match == 1) then
                    ...
                    dRijdRml = &
                     dRij_dRml(i, neighborindices(j), ri, Rj, m, l, rmin)
                    if (dRijdRml /= 0.0d0) then
                        Rij = sqrt(dot_product(Rij_vector, Rij_vector))
 
                        !Lei modified to avoid two atoms move too close
                        if (Rij < rmin) then
                           Rij = rmin
                        endif

                        if (present(p_gamma)) then
                            term1 = - 2.0d0 * g_eta * (Rij - rs) * &
                        ...
                        ridge = ridge + exp(- g_eta * ((Rij - rs)**2.0d0) / &
...
!     Returns 1 if try is the same set as val, 0 if not.
      ...
      function dRij_dRml(i, j, Ri, Rj, m, l, rmin)
              integer i, j, m, l
              double precision, dimension(3):: Ri, Rj, Rij_vector
              double precision:: dRij_dRml, Rij, rmin
              do xyz = 1, 3
                      Rij_vector(xyz) = Rj(xyz) - Ri(xyz)
              end do
              Rij = sqrt(dot_product(Rij_vector, Rij_vector))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      neighborpositions, g_numbers, g_gamma, g_zeta, g_eta, rc, theta_s, &
      cutofffn_code, i, ri, m, l, num_neighbors, ridge, p_gamma)
              ...
              integer:: i, m, l
              double precision:: g_gamma, g_zeta, g_eta, rc, theta_ijk
              ! gamma parameter for the polynomial cutoff
              double precision, optional:: p_gamma
              integer:: cutofffn_code
              double precision:: ridge, theta_s
...
!f2py         intent(in):: g_eta, rc, theta_s, ri, neighborindices , i, m, l
!f2py         intent(hide):: num_neighbors
!f2py         intent(out):: ridge
              integer:: j, k, match, xyz
              double precision, dimension(3):: Rij_vector, Rik_vector
              ...
              do j = 1, num_neighbors
                do k = (j + 1), num_neighbors
                  ...
                  if (match == 1) then
                    ...
                    theta_ijk = acos(costheta)
                    c1 = (1.0d0 + g_gamma * cos(theta_ijk - theta_s))
                    if (present(p_gamma)) then
                    ...
                    if (dCosthetadRml /= 0.d0) then
                      term2 = term2 + g_gamma * g_zeta * dCosthetadRml &
                           * sin( theta_ijk - theta_s )& 
                           / sqrt(1.0d0 - costheta **2.0d0)
                    end if
                    dRijdRml = &
