...
def save_to_openkim(calc, filename='amp.params', overwrite=False,
    ...
    desc_pars = calc.descriptor.parameters
    model_pars = calc.model.parameters
    print 'desc_pars:',desc_pars.keys()
    print 'model_pars:',model_pars.keys()
    print 'hiddenlayrs',model_pars['hiddenlayers']
    print 'fprange:', model_pars['fprange']
    if (desc_pars['mode'] != 'atom-centered' or
       model_pars['mode'] != 'atom-centered'):
        raise NotImplementedError(
        ...
        for G in desc_pars['Gs'][element]:
            if G['type'] == 'G2':
                ...
                f.write(G['elements'][0] + ' ' + G['elements'][1] + ' ' +
                        ...
                        str(G['zeta']) + ' ' + str(G['theta_s']) + 
                        '  # eta, gamma, zeta, theta_s')
            f.write('\n')
...
def tflow_save_to_openkim(calc, filename='amp.params', natoms=1, overwrite=False,
                    units="metal"):
    """Saves the calculator in a way that it can be used with OpenKIM.
    ...
    print 'desc_pars:',desc_pars.keys()
    print 'model_pars:',model_pars.keys()
    print 'hiddenlayrs',len(model.hiddenlayers)
    if desc_pars['mode'] != 'atom-centered':
        raise NotImplementedError(
            'KIM model requires atom-centered symmetry functions.')
    if desc_pars['cutoff']['name'] != 'Cosine':
        raise NotImplementedError(
...
#             filename, 'w')
    ...
    f.write(' '.join(str(len(desc_pars['Gs'][element])) for element in
            ...
            '  # number of fingerprints of each chemical species')
    f.write('\n')
    print "FPs:", desc_pars['Gs']
    print "model_pars['fprange']:", model_pars['fprange']

    for element in elements:
        count = 0

        #Reshape fprange:
        model_pars['fprange'][element] =  \
              np.array([model_pars['fprange'][element][0].tolist(), \
              model_pars['fprange'][element][1].tolist()]).T.tolist()
        print "fprange reshaped:", model_pars['fprange']
    ...
    f.write(str(cutoff) + '  # cutoff radius')
    f.write('\n')
    f.write(model.activation.__name__ + '  # activation function')
    f.write('\n')
    # writing the neural network structures
    hiddenlayers = model.hiddenlayers
    for element in elements:
        f.write(str(len(hiddenlayers)) +
                '  # number of hidden-layers of %s neural network' % element)
        f.write('\n')
        f.write(' '.join(str(_) for _ in hiddenlayers) +
                '  # number of nodes of hidden-layers of %s neural network' %
                element)
        f.write('\n')
