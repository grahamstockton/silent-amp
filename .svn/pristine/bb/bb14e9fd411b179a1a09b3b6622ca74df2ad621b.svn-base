...
!     the data of neuralnetwork (should be fed in by python)
      ...
      !Lei added to enable non-equal number of fps
      type :: fp_range_array
        double precision, dimension(:), allocatable:: min_fingerprints
        double precision, dimension(:), allocatable:: max_fingerprints
      end type fp_range_array
      type(fp_range_array), dimension(:), allocatable:: fp_range
...
!     Returns energy value in the atom-centered mode.
      ...
      len_of_fingerprint, num_elements, jm
      ...
      do l = 1, len_of_fingerprint
        ! Lei modified to enable non-equal number of fps
        if ((fp_range(element)%max_fingerprints(l) - &
        fp_range(element)%min_fingerprints(l)) .GT. &
        (10.0d0 ** (-8.0d0))) then
            fingerprint_(l) = -1.0d0 + 2.0d0 * &
            (fingerprint(l) - fp_range(element)%min_fingerprints(l)) / &
            (fp_range(element)%max_fingerprints(l) - &
            fp_range(element)%min_fingerprints(l))
        else
      ...
      do layer = 1, size(hiddensizes) + 1
          ...
          layer)%twodarray, dim=2)
              ...
              layer)%twodarray, dim=1)
                  ...
                  !Lei added
                  !if (m == 1) then
                  !    write(*,*) "Weights:",layer,(unraveled_parameters(&
                  !    element)%weights(layer)%twodarray(n, jm),jm = 1, 2)
                  !end if
              end do
              !Lei modified
              if ((activation_signal == 1) ) then
                  o(layer + 1)%onedarray(m) = tanh(net(m))
              else if ((activation_signal == 2) .and. (layer /= (size(hiddensizes) + 1))) then
                  !write(*,*) "Layer", layer
                  o(layer + 1)%onedarray(m) = &
                  1.0d0 / (1.0d0 +  exp(- net(m)))
              else if (activation_signal == 3) then
                  o(layer + 1)%onedarray(m) = net(m)
              !Lei added for tflow model
              else if (layer == (size(hiddensizes) + 1)) then
                  !write(*,*) "Linear:",layer
                  o(layer + 1)%onedarray(m) = net(m)
              end if
              ohat(layer + 1)%onedarray(m) = o(layer + 1)%onedarray(m)
      ...
      do layer = 1, nn + 1
          ...
          do p = 1, size(o(layer + 1)%onedarray)
              if (activation_signal == 1) then
                ...
                o(layer + 1)%onedarray(p))
              !Lei modified for tflow model
              else if ((activation_signal == 2) .and. (layer /= (nn + 1))) then
                doutputs_dinputs(layer + 1)%onedarray(p) = &
                temp(p) * (1.0d0 - o(layer + 1)%onedarray(p)) * &
                o(layer + 1)%onedarray(p)
              else if (activation_signal == 3) then
