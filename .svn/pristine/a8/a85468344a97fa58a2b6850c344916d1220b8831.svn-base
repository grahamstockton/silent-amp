...
#Lei added
from neuralnetwork import Raveler
...
class NeuralNetwork:
    ...
    def constructSessGraphModel(self, tfVars, sess, trainOnly=False,
        ...
        with self.graph.as_default():
            ...
            #Lei modified:
            self.trainvarlist = tf.trainable_variables()

            trainvarlist = [a for a in self.trainvarlist
    ...
    #Lei added
    #@property
    def vector(self, natoms):
        """
        Weights and bias stored in following format:
           'H': [w_fc0, b_fc0, w_fc1, b_fc1,...,w_fcout, b_fcout]
        """
        weights = OrderedDict()
        nameOrder = []
        for v in self.trainvarlist:
           var_name = v.name
           fields=var_name.split(':')[0].split('_')
           key = fields[1]
           try:
              numb_layer = int(fields[2][-1])
           except:
              if fields[2][:6] == 'Wfcout':
                 numb_layer += 1
              pass

           if key not in weights.keys():
              weights[key] = OrderedDict()
              weights[key][numb_layer]=self.sess.run(var_name).tolist()
              #weights[key][numb_layer]=[self.sess.run(var_name).flatten().tolist()]
           else:
              if numb_layer not in weights[key].keys():
                 weights[key][numb_layer]=self.sess.run(var_name).tolist()
                 #weights[key][numb_layer]=[self.sess.run(var_name).flatten().tolist()]
              else:
                 weights[key][numb_layer].extend([self.sess.run(var_name).tolist()])
                 #weights[key][numb_layer].extend(self.sess.run(var_name).flatten().tolist())
        scalings = OrderedDict()
        for key in weights.keys():
            scalings[key] = OrderedDict()
            scalings[key]['intercept'] = self.parameters['energyMeanScale']/ float(natoms)
            scalings[key]['slope'] = self.parameters['energyProdScale']
        if not hasattr(self, 'ravel'):
            self.ravel = Raveler(weights, scalings)
        return self.ravel.to_vector(weights=weights, scalings=scalings)
